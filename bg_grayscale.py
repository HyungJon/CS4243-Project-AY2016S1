...
def imgThresholding(differences, img, threshold = 40):
	newImg = np.copy(img)
	for r in range(img.shape[0]):
		for c in range(img.shape[1]):
			if differences[r][c] <= threshold: # only sufficiently different pixels are considered as foreground objects
				newImg[r][c] = 0 # replace background with black color
...
_, img = cap.read() 
avgImg = np.float32(rgb2gray(img)) # destination array
